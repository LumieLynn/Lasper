...
impl IStep for StorageStep {
    ...
    fn next_step(&self, _context: &WizardContext) -> Option<Box<dyn IStep>> {
        Some(Box::new(crate::ui::wizard::steps::user::UserStep::new()))
    }
    ...
    fn render(&mut self, f: &mut Frame, area: Rect, context: &WizardContext) {
        ...
        for (i, (st, supported)) in context.storage.info.types.iter().enumerate() {
            let is_selected = i == context.storage.type_idx;
        ...
        ScrollableList::new(" Storage Options ", items)
            .selected(if context.storage.field_idx == 0 { Some(context.storage.type_idx) } else { None })
            .render(f, chunks[1]);

        let (st, supported) = context.storage.info.types[context.storage.type_idx];
        let is_raw = st == StorageType::Raw;

        if is_raw {
            let size_text = if context.storage.field_idx == 1 { format!("{}_", context.storage.raw_size) } else { context.storage.raw_size.clone() };
            Input::new(" Raw Image Size (e.g. 2G, 500M) ", &size_text)
                .focused(context.storage.field_idx == 1)
                .render(f, chunks[3]);

            let fs_text = if context.storage.field_idx == 2 { format!("{}_", context.storage.raw_fs) } else { context.storage.raw_fs.clone() };
            Input::new(" Filesystem Type (ext4, xfs, btrfs, etc.) ", &fs_text)
                .focused(context.storage.field_idx == 2)
                .render(f, chunks[4]);

            let part_text = "[ ] Create custom partition table (Advanced - Unselectable)";
            let part_style = if context.storage.field_idx == 3 { Color::Cyan } else { Color::DarkGray };
            f.render_widget(
        ...
        let path_hint = format!("Path: {}", st.get_path(&context.basic.name).display());
    ...
    async fn handle_key(&mut self, key: KeyEvent, context: &mut WizardContext) -> StepAction {
        let count = context.storage.info.types.len();
        let is_raw = context.storage.info.types[context.storage.type_idx].0 == StorageType::Raw;

        match key.code {
            KeyCode::Esc => StepAction::Prev,
            KeyCode::Up => {
                if context.storage.field_idx == 0 {
                    if context.storage.type_idx > 0 { context.storage.type_idx -= 1; }
                } else {
                    context.storage.field_idx -= 1;
                }
                StepAction::None
            }
            KeyCode::Down => {
                if context.storage.field_idx == 0 {
                    if context.storage.type_idx + 1 < count { context.storage.type_idx += 1; }
                } else {
                    let max_field = if is_raw { 3 } else { 0 };
                    if context.storage.field_idx < max_field { context.storage.field_idx += 1; }
                }
                StepAction::None
            }
            KeyCode::Tab => {
                let max_field = if is_raw { 3 } else { 0 };
                context.storage.field_idx = (context.storage.field_idx + 1) % (max_field + 1);
                StepAction::None
            }
            KeyCode::Backspace => {
                if is_raw {
                    match context.storage.field_idx {
                        1 => { context.storage.raw_size.pop(); }
                        2 => { context.storage.raw_fs.pop(); }
                        _ => {}
                    }
                }
                StepAction::None
            }
            KeyCode::Char(' ') | KeyCode::Enter if context.storage.field_idx == 3 => {
                StepAction::Status("Feature in progress: Custom partitioning is coming soon!".into(), StatusLevel::Info)
            }
            KeyCode::Char(c) => {
                if is_raw {
                    match context.storage.field_idx {
                        1 => { context.storage.raw_size.push(c); }
                        2 => { context.storage.raw_fs.push(c); }
                        _ => {}
                    }
                }
                StepAction::None
