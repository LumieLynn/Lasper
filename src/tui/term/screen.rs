    ...
    fn text(&mut self, c: char) {
        ...
        if pos.col > size.width.saturating_sub(width) {
            ...
            let last_cell = self
                ...
                .unwrap();
            if last_cell.has_contents() || self.grid().is_wide_continuation(last_cell_pos) {
                wrap = true;
            }
        }
        ...
        if width == 0 {
            ...
            if self
                ...
                .is_wide()
            {
                if let Some(next_cell) = self.grid_mut().drawing_cell_mut(super::grid::Pos {
                    row: pos.row,
                    col: pos.col + 1,
                }) {
                    next_cell.set(' ', attrs);
                    ...
                    match buf[pos - 1] {
                        ...
                        b'[' => {
                            ...
                            if (0x40..=0x7E).contains(final_) {
                                ...
                                let intermediate = str::from_utf8(intermediate).unwrap_or_default();
                        ...
                        c => {
                            log::warn!("Unhandled ESC {} ({:?})", c, char::from_u32(c.into()));
                        }
                    }
                }
                0x8D => {
...
mod tests {
    ...
    use crate::tui::term::common::Size;
