use crate::nspawn::adapters::storage::{ImageMountSource, ManagedImageKind};
...
async fn run_command_with_stdin(
    ...
    let mut child = crate::nspawn::sys::new_command(program)
        .args(args)
...
pub(crate) fn probe_image_partitions(image: &Path) -> Result<Option<ImagePartitionProbe>> {
    let output = crate::nspawn::sys::new_sync_command("sfdisk")
        .arg("--json")
        .arg(image)
        .output()
        .map_err(|error| command_io_error("sfdisk", error))?;
