...
impl SessionPort for DirectSessionAdapter {
    async fn open_terminal(
        ...
        let attachment = match self.terminal_policy {
            DirectTerminalPolicy::LoginOnly => {
                crate::nspawn::sys::terminal_attach::login(&request.machine)
            }
            DirectTerminalPolicy::Automatic => {
                crate::nspawn::sys::terminal_attach::select(&request.machine).map_err(|error| {
                    SessionError::new(format!("plan terminal attachment: {error}"))
                })?
            }
        };
        let kind = attachment.kind();
        crate::nspawn::adapters::comm::pty::spawn_direct_terminal(
    ...
    async fn open_journal(
        ...
        let mut child = crate::nspawn::sys::new_command("journalctl")
            .args([
                "-M",
