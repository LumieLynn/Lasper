...
impl NspawnConfig {
    ...
    pub async fn update_gpu_passthrough(
        name: &str,
        new_state: &super::nvidia::NvidiaState,
        death_list: &[String],
    ) -> Result<()> {
        let path = PathBuf::from(format!("/etc/systemd/nspawn/{}.nspawn", name));
        let content =
...
mod tests {
    ...
    #[test]
    fn test_purge_nvidia_block_clean() {
        let content = "[Network]\nBridge=br0\n";
        let (clean, death_list) = NspawnConfig::purge_nvidia_block(content).unwrap();
        assert_eq!(clean, content);
        assert!(death_list.is_empty());
    }

    #[test]
    fn test_purge_nvidia_block_existing() {
        let content = "[Network]\nBridge=br0\nX-Lasper-Nvidia-Begin=550.54\nBind=/dev/nvidia0\nBindReadOnly=/usr/lib/libcuda.so.1\nX-Lasper-Nvidia-End=true\n[Exec]\nUser=root";
        let (clean, death_list) = NspawnConfig::purge_nvidia_block(content).unwrap();
        assert!(clean.contains("[Network]"));
        assert!(clean.contains("[Exec]"));
        assert!(!clean.contains("X-Lasper-Nvidia-Begin"));
        assert_eq!(death_list, vec!["/dev/nvidia0", "/usr/lib/libcuda.so.1"]);
    }

    #[test]
    fn test_purge_nvidia_block_corrupted_missing_end() {
        let content = "X-Lasper-Nvidia-Begin=550.54\nBind=/dev/nvidia0";
        let res = NspawnConfig::purge_nvidia_block(content);
        assert!(res.is_err());
    }

    #[test]
    fn test_purge_nvidia_block_corrupted_double_begin() {
        let content =
            "X-Lasper-Nvidia-Begin=550.54\nX-Lasper-Nvidia-Begin=555.42\nX-Lasper-Nvidia-End=true";
        let res = NspawnConfig::purge_nvidia_block(content);
        assert!(res.is_err());
    }

    #[test]
    fn test_apply_gpu_passthrough_with_dedup() {
        let content = "[Files]\nBind=/dev/nvidia0\n[Exec]\nBoot=yes";
