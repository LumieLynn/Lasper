...
pub(crate) fn map_native_error(error: crate::nspawn::errors::NspawnError) -> MachineControlOutcome {
    ...
    match error {
        ...
        NspawnError::Dbus(zbus::Error::MethodError(name, detail, _)) => {
            let rejection = match name.as_str() {
                ...
                | "System.Error.EACCES"
                | "System.Error.EPERM" => Some(MachineRejection::PermissionDenied),
                "org.freedesktop.DBus.Error.InvalidArgs" | "System.Error.EINVAL" => {
                    Some(MachineRejection::InvalidTarget)
                }
                _ => None,
            };
            match rejection {
