...
impl StorageType {
    ...
    pub fn get_path(&self, name: &str) -> PathBuf {
        match self {
            Self::Directory | Self::Subvolume => PathBuf::from(format!("/var/lib/machines/{}", name)),
...
fn get_filesystem_type(path: &Path) -> Result<String> {
    ...
    if out.status.success() {
        ...
        Err(NspawnError::CommandFailed("stat".into(), String::from_utf8_lossy(&out.stderr).trim().to_string()))
    }
}
...
impl StorageBackend for DirectoryBackend {
    fn get_type(&self) -> StorageType { StorageType::Directory }
    fn get_path(&self, name: &str) -> PathBuf { PathBuf::from(format!("/var/lib/machines/{}", name)) }
    
    async fn create(&self, name: &str) -> Result<PathBuf> {
        let path = self.get_path(name);
        tokio::fs::create_dir_all(&path).await.map_err(|e| NspawnError::Io(path.clone(), e))?;
        Ok(path)
    }
    ...
    async fn delete(&self, name: &str) -> Result<()> {
        let path = self.get_path(name);
        if path.exists() {
            tokio::fs::remove_dir_all(&path).await.map_err(|e| NspawnError::Io(path, e))?;
        }
        Ok(())
    }
...
impl StorageBackend for SubvolumeBackend {
    fn get_type(&self) -> StorageType { StorageType::Subvolume }
    fn get_path(&self, name: &str) -> PathBuf { PathBuf::from(format!("/var/lib/machines/{}", name)) }
    ...
    async fn create(&self, name: &str) -> Result<PathBuf> {
        ...
        if !out.status.success() {
            return Err(NspawnError::CommandFailed("btrfs subvolume create".into(), String::from_utf8_lossy(&out.stderr).to_string()));
        }
        Ok(path)
    }
    ...
    async fn delete(&self, name: &str) -> Result<()> {
        ...
        if path.exists() {
            let out = Command::new("btrfs")
                ...
                .map_err(|e| NspawnError::Io(PathBuf::from("btrfs"), e))?;
            if !out.status.success() {
                return Err(NspawnError::CommandFailed("btrfs subvolume delete".into(), String::from_utf8_lossy(&out.stderr).to_string()));
            }
        }
        Ok(())
    }
...
impl StorageBackend for RawBackend {
    fn get_type(&self) -> StorageType { StorageType::Raw }
    fn get_path(&self, name: &str) -> PathBuf { PathBuf::from(format!("/var/lib/machines/{}.raw", name)) }
    ...
    async fn create(&self, name: &str) -> Result<PathBuf> {
        ...
        if !out.status.success() {
            return Err(NspawnError::CommandFailed("truncate".into(), String::from_utf8_lossy(&out.stderr).to_string()));
        }
        ...
        if !out.status.success() {
            return Err(NspawnError::CommandFailed(mkfs_prog, String::from_utf8_lossy(&out.stderr).to_string()));
        }
        Ok(path)
    }
    ...
    async fn mount(&self, name: &str) -> Result<PathBuf> {
        ...
        // Create mount point
        tokio::fs::create_dir_all(&mount_point).await.map_err(|e| NspawnError::Io(mount_point.clone(), e))?;
        
        // Find/Setup loop device and mount in one go using 'mount -o loop'
        let out = Command::new("mount")
            .args(["-o", "loop", &img_path.to_string_lossy(), &mount_point.to_string_lossy()])
            .output()
            .await
            .map_err(|e| NspawnError::Io(PathBuf::from("mount"), e))?;
        
        if !out.status.success() {
            return Err(NspawnError::CommandFailed("mount".into(), String::from_utf8_lossy(&out.stderr).to_string()));
        }
    ...
    async fn delete(&self, name: &str) -> Result<()> {
        let path = self.get_path(name);
        if path.exists() {
            tokio::fs::remove_file(&path).await.map_err(|e| NspawnError::Io(path, e))?;
        }
        Ok(())
    }
